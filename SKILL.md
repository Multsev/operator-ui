...
Default to loopback-only publication. Require an explicit security decision before binding to a LAN or public interface. A LAN mode must bind only the selected private interface, use HTTPS, issue a host/IP-matching certificate from an application-owned CA, keep the CA private key in the OS credential vault, and provide an explicit public-certificate installation flow for client devices. Keep access tokens outside URLs sent to the server, logs, localStorage and application source; use an URL fragment only for one-time browser bootstrap, then remove it and retain the token in session storage. Require TLS or a trusted reverse proxy outside loopback; never silently downgrade LAN access to HTTP.
